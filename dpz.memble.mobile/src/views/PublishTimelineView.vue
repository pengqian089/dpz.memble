<template>
  ...
  <van-nav-bar title="发布时间轴" left-arrow left-text="返回" @click="back"/>
...
import {handleUploadResponse, warning} from "@/common";

export default {
  name: "PublishTimelineView",
  components: {MdEditor},
  data() {
    return {
      id: null,
  ...
  methods: {
    async publish(values) {
      ...
      Notify({type:"success",message:"发布成功，正在跳转至时间轴列表"})
